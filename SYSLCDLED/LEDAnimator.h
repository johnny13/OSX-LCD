...
private:
    void checkHighStress(uint32_t now) {
        ...
        if (cpuHigh) {
            ...
            highCpuStartTime = 0;
            if (isHighStress) {
                phase = returnPhase; // Now safely drops the alert phase back to normal!
                phaseStart = now;
                isHighStress = false;
            }
        }
    ...
    void renderTempAlert(uint32_t now) {
        ...
        if (tempFlashCount < 6) {
            ...
            if (tempFlashOn) renderSolidHSV(0, 0, 255); // Flash White
            else renderSolidHSV(0, 0, 0);               // Flash Off
            return;
        }
